<?php
/**
 * Plugin Name:       Personalize Login (Rocketship)
 * Description:       A plugin that replaces the WordPress login flow with a custom page by Rocketship bssed on Jarkko Laine
 * Version:           1.0.0
 * Author:            Alex Burca
 * License:           GPL-2.0+
...
class Personalize_Login_Plugin {
	...
	public function __construct() {
        ...
        add_filter( 'lostpassword_url', array($this, 'replace_password_url'), 90, 2 );
        ...
        add_shortcode( 'custom-login-area-forms', array($this, 'handle_all_custom_forms'));
	}
	...
	public function redirect_to_custom_lostpassword() {
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			wp_redirect( home_url( '/?action='.$this->getParams['lost'] ) );
			exit;
		}
	}
	...
	public function redirect_to_custom_password_reset() {
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			$redirect_url = home_url( '/' );
			$redirect_url = add_query_arg( 'action', $this->getParams['reset'], $redirect_url );
    ...
    public function handle_all_custom_forms(){
        if(isset($_GET['action']) && $_GET['action'] === 'lostpassword'){
            return do_shortcode('[custom-password-lost-form]');
        }
        if(isset($_GET['action']) && $_GET['action'] === 'resetpassword'){
            return do_shortcode('[custom-password-reset-form]');
        }
        return do_shortcode('[custom-login-form]');
    }
	...
	public function do_password_lost() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( is_wp_error( $errors ) ) {
				// Errors found
				$redirect_url = home_url( '/?action='.$this->getParams['lost'] );
    ...
    public function replace_password_url($lostpassword_url, $redirect){
        return '/?action='.$this->getParams['lost'];
    }
}
